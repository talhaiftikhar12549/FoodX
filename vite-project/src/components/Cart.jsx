...
export default function Cart()
{
    ...
    return(
        <div >
            <div className="flex w-[50%] items-center justify-center text-center mx-auto p-4 border rounded-lg shadow-lg px-12 py-12 my-4 ">

                <div className="w-[95%] "><p className="text-4xl font-black text-[#98C3ED] font-serif  pb-6  ">Your Cart</p>
                    ...
                    {cartItem && cartItem.map((abc) => (
                            <div className="border rounded-lg shadow-lg my-4" key={abc.id}>
                                <div className="flex ">
                                    <div className="w-[30%]">
                                        <img src={menupic} className="h-[150px] w-[150px]"/>
                                    </div>
                                    <div id="centerwalli"
                                         className="w-[50%] flex flex-col justify-start text-left pl-2 py-2 h-[90%]">
                                        <p className="font-semibold text-lg font-sans">{abc.name}</p>
                                        <div className="mt-8">
                                            <div className="font-semibold text-md">Quantity</div>
                                            <div
                                                className="py-[6px] px-[20px] border-[2px] border-[#98c3ed] text-[#98c3ed] shadow-lg font-medium my-2 flex justify-between max-w-[40%]">
                                                <button onClick={()=>(dispatch(decItem({ id: abc.id, name: abc.name, desc: abc.description, price: abc.price } )))} className="h-[100%]">-</button>
                                                {abc.quantity}
                                                <button onClick={()=>(dispatch(incItem({ id: abc.id, name: abc.name, desc: abc.description, price: abc.price })))} className="h-[100%]">+</button>
                                            </div>
                                        </div>
                                    </div>
